...
# See https://docs.google.com/spreadsheets/d/1PW0QnOKzDnuU1TDaSX4AGA4HbJsSXJZBXPqTN8CkXKA/edit#gid=0

#Base population is all cases in the system
#VIP is based on date they became a VIP

#Dataset of all non-test accounts and all cases within those accounts that currently exist (10/1/23 to now) to get set of unique people
#Use that set to merge in caseIDs of true clients to create list of unique caseIDs
#VIP to be per iteration of treatment (based on caseID)
...
# SF query parameters
gv_VIPsource1="Account"
gv_VIPfields1="Id, Name, CreatedDate, Test_Account__c"

gv_VIPsource2="Form_Data__c"
gv_VIPfields2="Id,Account__c,Care_Plan__c, CreatedDate, CRISIS_Crisis_Reasons__c, CRISIS_Suicide_Timing__c, Aborted_Suicide_Prep__c, Interrupted_Prep__c, Recent_Suicide_Attempt__c, BSCS_Self_Worth__c, BSCS_Problem_Help__c, BSCS_Coping_Struggle__c, BSCS_Enduring_Pain__c, BSCS_Self_Redeeming__c, BSCS_Suicide_Thoughts__c, CRP_Firearms_Access__c, CRP_Safe_lockbox_Usage__c, CRP_Firearms_Loaded__c,  IIA_Outcome__c, IIA_Alternate_Methods__c,  IIA_Similar_Trigers_or_Methods__c, COQ_B_Talked_About_Suicide__c, IIA_Ability_to_Maintain_Safety__c, COQ_Fleeting_Thoughts__c" 
###Name=="B-COQ: Baseline Core Outcomes Questionnaire"

gv_VIPsource3="CaseHistory"
gv_VIPfields3="CaseId, Field, NewValue, CreatedDate" 

gv_VIPsource4="Case"
gv_VIPfields4= "Id, AccountID, I_am__c, Thoughts_Intention_of_acting_on_them__c, Details_of_How_Intend_to_Carry_Out__c, Done_Started_Prepared_last_3_months__c, Any_thoughts_of_killing_yourself__c,Wished_you_were_dead_Past_Month__c,Thoughts_of_killing_yourself_past_month__c, Done_Started_Prepared_to_end_your_life__c, Status_Change_Reason__c, Status, CreatedDate"

gv_VIPsource5="Case_Note__c"
gv_VIPfields5="Id, Overall_Risk_Level__c, Risk_Level_Change_Reason__c, CreatedDate" 
```

```{r Queries}
VIPaccount_query=sprintf(paste("SELECT ",gv_VIPfields1," FROM ",gv_VIPsource1," WHERE CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate))
VIPaccount=sf_query(VIPaccount_query)

VIPform_query=sprintf(paste("SELECT ",gv_VIPfields2," FROM ",gv_VIPsource2," WHERE CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate))
VIPform=sf_query(VIPform_query)

VIPcaseh_query=sprintf(paste("SELECT ",gv_VIPfields3," FROM ",gv_VIPsource3," WHERE CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate))
VIPcaseh <- sf_query(VIPcaseh_query)

VIPhistory_query=sprintf(paste("SELECT ",gv_VIPfields3," FROM ",gv_VIPsource3," WHERE CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate))
VIPhistory=sf_query(VIPhistory_query)

VIPcase_query=sprintf(paste("SELECT ",gv_VIPfields4," FROM ",gv_VIPsource4," WHERE CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate))
VIPcase=sf_query(VIPcase_query)

VIPcasenote_query=sprintf(paste("SELECT ",gv_VIPfields5," FROM ",gv_VIPsource5," WHERE CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate))
VIPcasenote <- sf_query(VIPcasenote_query)
```

```{r Merge & Filter}

#Remove all rows where the case does not have an associated account or requests were made by a third party
...
VIPcas_acc=merge(VIPcase,VIPaccount,by="AccountId") %>%
filter(is.na(AccountId)==FALSE, (is.na(I_am__c)|I_am__c!="I am requesting help for a veteran/service member"),((is.na(Status_Change_Reason__c)==TRUE & Status=="Active_In_Care")|(Status_Change_Reason__c!="Third Party - Did Not Convert Client" & Status_Change_Reason__c!="Third Party - Converted Client" & Status_Change_Reason__c!="Duplicate Case for Active Client" & Status_Change_Reason__c!="Test Case" & Status_Change_Reason__c!="Ineligible - Not a Service Member/Veteran" & Status_Change_Reason__c!="Ineligible - Underage" & Status_Change_Reason__c!="Declined Services At Intake" & Status_Change_Reason__c!="Declined Services At Scheduling" & Status_Change_Reason__c!="Referred to External Provider" & Status_Change_Reason__c!="Unable to Contact - Intake Not Completed, Client Nonconnection" & Status_Change_Reason__c!="Caring Contacts - Ongoing" & Status_Change_Reason__c!="Caring Contacts - Completed" & Status_Change_Reason__c!="Caring Contacts - Client Returned to Care" & Status_Change_Reason__c!="Caring Contacts - Client Opted Out")))%>%
  ...
  filter(testname==0,rrbcname==0,Test_Account__c==FALSE,(is.na(Status_Change_Reason__c)|(Status_Change_Reason__c!="Duplicate Case for Active Client" & Status_Change_Reason__c!="Test Case" & Status_Change_Reason__c!="Ineligible - Not a Service Member/Veteran" & Status_Change_Reason__c!="Ineligible - Underage"))) 

#Important exclusion criteria!
#Remove cases without an account, and 3P requests
#inner join, only keep cases that have an account associated with it.
...
```{r}
#Merge used for all VIP criteria using the Form Data Object
VIP_form_df=merge(VIPcas_acc, VIPform, by="CaseId",all.x=TRUE) #keep all the original cases even if they do not have form data

unique(VIPcas_acc$CaseId)
unique(VIPcas_acc$AccountId)

backup=VIP_form_df

#Merge used for all VIP criteria using the CaseHistory Data Object
VIP_caseh_df=merge(VIPcas_acc,VIPcaseh,by="CaseId",all.x=TRUE) #keep all the original cases even if they do not have form data

##VIP01: Form Data; Done
VIP1_form_df = VIP_form_df %>% 
  mutate(VIP01 = case_when(grepl("Lethal Means in Hand", CRISIS_Crisis_Reasons__c, fixed=T) | grepl("Active Attempt", CRISIS_Crisis_Reasons__c, fixed=T) | grepl("Suicidal Gestures", CRISIS_Crisis_Reasons__c, fixed=T)  ~ 1, TRUE ~ NA_real_)) %>%
    filter(VIP01==1) %>%
  select(CaseId, CreatedDate, VIP01) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP01.Date=CreatedDate)

##VIP02: Form Data; Done
VIP2_form_df = VIP_form_df %>% 
  mutate(VIP02 = case_when(CRISIS_Suicide_Timing__c=="within the next day" | CRISIS_Suicide_Timing__c=="within 7 days" | CRISIS_Suicide_Timing__c=="within 30 days"  ~ 1, TRUE ~ NA_real_))%>%
    filter(VIP02==1) %>%
  select(CaseId, CreatedDate, VIP02) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP02.Date=CreatedDate)

##VIP03: Form Data; Done
VIP3_form_df = VIP_form_df %>% 
  mutate(VIP03 = case_when(Aborted_Suicide_Prep__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
    filter(VIP03==1) %>%
  select(CaseId, CreatedDate, VIP03) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP03.Date=CreatedDate)

##VIP04: Form Data; Done
VIP4_form_df = VIP_form_df %>% 
  mutate(VIP04 = case_when(Interrupted_Prep__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
    filter(VIP04==1) %>%
  select(CaseId, CreatedDate, VIP04) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP04.Date=CreatedDate)

##VIP05: Form Data; Done
VIP5_form_df = VIP_form_df %>% 
  mutate(VIP05 = case_when(Recent_Suicide_Attempt__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
    filter(VIP05==1) %>%
  select(CaseId, CreatedDate, VIP05) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP05.Date=CreatedDate)

##VIP06: Case, Case History; Confirmed
VIP6_caseh_df = VIP_caseh_df %>% 
  mutate(VIP06 = case_when(Status_Change_Reason__c=="Died by Suicide"  ~ 1, TRUE ~ NA_real_)) %>%
filter(VIP06==1) %>%
  select(CaseId, CreatedDate, VIP06) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP06.Date=CreatedDate)

##VIP07: Case; Confirmed
VIP7_case_df = VIPcase %>% 
  mutate(VIP07 = case_when(Thoughts_Intention_of_acting_on_them__c=="Yes" | Details_of_How_Intend_to_Carry_Out__c=="Yes" | Done_Started_Prepared_last_3_months__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
  filter(VIP07==1) %>%
  select(CaseId, CaseCreatedDate, VIP07) %>%
   group_by(CaseId) %>%
  filter(CaseCreatedDate == min(CaseCreatedDate)) %>%
    rename(VIP07.Date=CaseCreatedDate)

##VIP08: Form Data; Done
VIP8_form_df = VIP_form_df %>% 
  rowwise() %>% 
  mutate(VIP08 = sum(c_across(BSCS_Coping_Struggle__c:BSCS_Suicide_Thoughts__c), na.rm = T))  %>% 
  mutate(VIP08 = case_when(VIP08 > 12 ~ 1, TRUE ~ NA_real_)) %>%
    filter(VIP08==1) %>%
  select(CaseId, CreatedDate, VIP08) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP08.Date=CreatedDate)

##VIP09: Case History; Confirmed
VIP9_caseh_df = VIP_caseh_df %>% 
  mutate(VIP09 = case_when(Field=="Most Recent Overall Risk Level" & NewValue=="HRC"  ~ 1, TRUE ~ NA_real_)) %>%
  filter(VIP09==1) %>%
  select(CaseId, CreatedDate, VIP09) %>%
   group_by(CaseId) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP09.Date=CreatedDate)

##VIP10: Case/Form Data; Confirmed
VIP10_form_df = VIP_form_df %>% 
  mutate(VIP10 = case_when(Any_thoughts_of_killing_yourself__c=="Yes" | Wished_you_were_dead_Past_Month__c=="Yes" | Thoughts_of_killing_yourself_past_month__c=="Yes" | Done_Started_Prepared_to_end_your_life__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
    mutate(VIP10 = case_when(Name=="B-COQ: Baseline Core Outcomes Questionnaire" & COQ_Fleeting_Thoughts__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
    mutate(VIP10 = case_when((IIA_Ability_to_Maintain_Safety__c=="Intent to Act Not Found, Completed CRP" |   IIA_Ability_to_Maintain_Safety__c=="Intent to Act Unclear, Completed Contingency Plan" | IIA_Ability_to_Maintain_Safety__c=="Intent to Act Is Present, Contacted Emergency Services") & COQ_B_Talked_About_Suicide__c=="No" ~ 1, TRUE ~ NA_real_ )) %>%
  filter(VIP10==1) %>%
  select(CaseId, CaseCreatedDate, VIP10) %>%
   group_by(CaseId) %>%
  filter(CaseCreatedDate == min(CaseCreatedDate)) %>%
    rename(VIP10.Date=CaseCreatedDate)

##VIP11: Form Data; Confirmed
VIP11_form_df = VIP_form_df %>% 
  mutate(VIP11 = case_when(CRP_Firearms_Access__c=="Yes" & CRP_Safe_lockbox_Usage__c=="No" & CRP_Firearms_Loaded__c=="Yes" ~ 1, TRUE ~ NA_real_)) %>% 
  #mutate(VIP11 = case_when(grepl("firearm", IIA_Outcome__c,ignore.case = TRUE) ~ "1",
                            #grepl("firearm", IIA_Alternate_Methods__c, ignore.case = TRUE) ~"1",
                            #grepl("firearm", IIA_Similar_Trigers_or_Methods__c, ignore.case = TRUE) ~"1",
                            #grepl("gun", IIA_Outcome__c,ignore.case = TRUE) ~ "1",
                            #grepl("gun", IIA_Alternate_Methods__c, ignore.case = TRUE) ~"1",
                            #grepl("gun", IIA_Similar_Trigers_or_Methods__c, ignore.case = TRUE) ~"1")) %>%
          filter(VIP11==1) %>%
          select(CaseId, CreatedDate, VIP11) %>%
          group_by(CaseId) %>%
          filter(CreatedDate == min(CreatedDate)) %>%
          rename(VIP11.Date=CreatedDate)

#Merge all VIP measures together
VIPsimp=VIPcas_acc %>%
select(AccountId, AccountCreatedDate, CaseId, CaseCreatedDate)

VIP_merged= left_join(VIPsimp, VIP1_form_df, by='CaseId') %>%
                left_join(., VIP2_form_df, by='CaseId') %>%
                left_join(., VIP3_form_df, by='CaseId') %>%
                left_join(., VIP4_form_df, by='CaseId') %>%
                left_join(., VIP5_form_df, by='CaseId') %>%
                left_join(., VIP6_caseh_df, by='CaseId') %>%
                left_join(., VIP7_case_df, by='CaseId') %>%
                left_join(., VIP8_form_df, by='CaseId') %>%
                left_join(., VIP9_caseh_df, by='CaseId') %>%
                left_join(., VIP10_form_df, by='CaseId') %>%
                left_join(., VIP11_form_df, by='CaseId') %>%
                rowwise() %>% 
                mutate(VIP_client=case_when(VIP01==1 |VIP02==1|VIP03==1 |VIP04==1 |VIP05==1 |VIP06==1 |VIP07==1 |VIP08==1 |VIP09==1 |VIP10==1 |VIP11==1 ~ 1)) %>%
              #find earliest and latest date within each row
                 mutate(EarliestVIPdate = pmin(VIP01.Date, VIP02.Date, VIP03.Date, VIP04.Date, VIP05.Date, VIP06.Date, VIP07.Date, VIP08.Date, VIP09.Date, VIP10.Date, VIP11.Date, na.rm = TRUE), 
                          LatestVIPdate = pmax(VIP01.Date, VIP02.Date, VIP03.Date, VIP04.Date, VIP05.Date, VIP06.Date, VIP07.Date, VIP08.Date, VIP09.Date, VIP10.Date, VIP11.Date, na.rm = TRUE)) %>%
                select(-contains(".Date"))
                
##VIP12

############################################################################
##Numbers for Quarterly USAA Reporting##

###Identify the unique number of Accounts/Cases in any of the reports above
VIP_only= VIP_merged %>% 
    filter(VIP_client==1) 

#Add in all 0's for NAs

#of unique accounts with VIP designation
nrow(distinct(VIP_only,AccountId))
#of unique cases with VIP designation
nrow(VIP_only)

#Frequency table by VIPFlag (Which flags are most common)
    #ctable for cross-tabulation
dfSummary(VIP_only)


library(lubridate)
VIP_only$month.early <- format(as.Date(VIP_only$EarliestVIPdate, format="%Y/%m/%d"),"%m")
VIP_only$month.late <- format(as.Date(VIP_only$LatestVIPdate, format="%Y/%m/%d"),"%m")


VIP_only %>% group_by(month.early) %>% summarise(count = length(VIP_client[!is.na(VIP_client)]))
VIP_only %>% group_by(month.late) %>% summarise(count = length(VIP_client[!is.na(VIP_client)]))

VIP_only %>%
group_by(AccountId, month.early) %>% summarise(count = length(VIP_client[!is.na(VIP_client)]))

#########################################
##Dashboard Build##

###Show number of unique clients/Accounts by month (using VIPDate) and overall for a selected time period for each of the categories above
  ##Using EarliestVIPDate per account 
  
###Also show the total number of unique clients/Accounts who meet any of the criteria above over the timeframe specified and show the number by month of the first time they were flagged as a VIP

###Create demographic charts for all the accounts who are flagged as a VIP in the selected timeframe. Fields on the Account record would be: Age, Most Recent Overall Risk Level, Gender, Race, Ethnicity, Marital Status, Branch of Service, Service Status, Homeless, Employment Status, Domestic Violence, State (these are for establishing marketing profiles to those who are VIPs)

###At the bottom include a list of the actual clients who met any of the criteria for VIP, and maybe which VIP criteria were met, that would allow managers to click on accounts and explore the clients records in greater detail.

```

#Testing VIP Filters
```{r}
test=read.csv("test_filters.csv")

##VIP01: Form Data
test = test %>% 
  mutate(VIP01 = case_when(CRISIS_Crisis_Reasons__c=="Lethal Means in Hand" | CRISIS_Crisis_Reasons__c=="Active Attempt" | CRISIS_Crisis_Reasons__c=="Suicidal Gestures"  ~ 1, TRUE ~ NA_real_))
...
test = test %>% 
  rowwise() %>% 
  mutate(VIP011 = sum(c_across(BSCS_Coping_Struggle__c:BSCS_Suicide_Thoughts__c), na.rm = T))  %>% 
  mutate(VIP011 = case_when(VIP011 > 12 ~ 1, TRUE ~ NA_real_))

##VIP09: Case History
test = test %>% 
  mutate(VIP09 = case_when(Field=="Most Recent Overall Risk Level" & NewValue=="HRC"  ~ 1, TRUE ~ NA_real_))

#VIP10: Form/Case Data
test = test %>% 
 mutate(VIP10 = case_when(Any_thoughts_of_killing_yourself__c=="Yes" | Wished_you_were_dead_Past_Month__c=="Yes" | Thoughts_of_killing_yourself_past_month__c=="Yes" | Done_Started_Prepared_to_end_your_life__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
  mutate(VIP10 = case_when(Name=="B-COQ: Baseline Core Outcomes Questionnaire" & COQ_Fleeting_Thoughts__c=="Yes"  ~ 1, TRUE ~ NA_real_)) %>%
   mutate(VIP10 = case_when((IIA_Ability_to_Maintain_Safety__c=="Intent to Act Not Found, Completed CRP" | IIA_Ability_to_Maintain_Safety__c=="Intent to Act Unclear, Completed Contingency Plan" | IIA_Ability_to_Maintain_Safety__c=="Intent to Act Is Present, Contacted Emergency Services") & COQ_B_Talked_About_Suicide__c=="No" ~ 1, TRUE ~ NA_real_ )) 
...
##VIP11: Form Data
test = test %>% 
  mutate(VIP011 = case_when(CRP_Firearms_Access__c=="Yes" & CRP_Safe_lockbox_Usage__c=="No" & CRP_Firearms_Loaded__c=="Yes" ~ 1, TRUE ~ NA_real_)) %>% 
  mutate(VIP011 = case_when(grepl("firearm", IIA_Outcome__c,ignore.case = TRUE) ~ "1",
                            grepl("firearm", IIA_Alternate_Methods__c, ignore.case = TRUE) ~"1",
                            grepl("firearm", IIA_Similar_Trigers_or_Methods__c, ignore.case = TRUE) ~"1",
                            grepl("gun", IIA_Outcome__c,ignore.case = TRUE) ~ "1",
                            grepl("gun", IIA_Alternate_Methods__c, ignore.case = TRUE) ~"1",
...
```{r}
test.date=read.csv("test_date.csv")

##VIP01: Form Data; Confirmed
test.date = test.date %>% 
  mutate(VIP01 = case_when(CRISIS_Crisis_Reasons__c=="Lethal Means in Hand" | CRISIS_Crisis_Reasons__c=="Active Attempt" | CRISIS_Crisis_Reasons__c=="Suicidal Gestures"  ~ 1, TRUE ~ NA_real_)) %>%
    filter(VIP01==1) %>%
  select(AccountID, CaseID, CreatedDate, VIP01) %>%
   group_by(CaseID) %>%
  filter(CreatedDate == min(CreatedDate)) %>%
    rename(VIP01.Date=CreatedDate)
```
...
#Sections below here are still under development. 
#Potential avenues for visualization/interpretation
```{r}

#By date (Note: Dates aren't particularly helpful)
AP_full$month <- format(as.Date(AP_full$AP_date, format="%Y/%m/%d"),"%m")
AP_full %>% group_by(month) %>% summarise(total = sum(AP_max[!is.na(AP_max)]))

#By date of when they became a client?: 
...
#Non-Response Issues/Weighting
```{r}
#calculate non-response rate for the columns we use in the code
#which ones are effected by non-response
#what SOPs are playing a role
#what is the magnitude of the issue


```

#Read in case history
```{r}
#We will likely not use this in the future
#The history field does not contain all of the useful information for this calculation. When we try to calculate PA using this object, we grep for terms that can be used in both positive and negative ways, which leads to assignment of PA values that are invalid. This issue is eliminated by using the CaseNote object, which is what we have opted for below. This code is saved here, but can be disregarded at this time. 

#Case history
#gv_APsource3="CaseHistory"
#gv_APfields3="CaseId, Field, NewValue, CreatedDate" 

#Only bring in the case History where the caseid matches our apcas_acc file as well
#APcaseh_query=sprintf(paste("SELECT ",gv_APfields3," FROM ",gv_APsource3," WHERE Field='Most_Recent_Overall_Risk_Level__c' and CreatedDate>=",gv_startdate," and CreatedDate<",gv_enddate," and CaseId IN (",paste("'",unique(APcas_acc$CaseId),"'",sep="",collapse=","),")"))
#APcaseh=sf_query(APcaseh_query)

#PA09.: Revised - CaseHistory 

#CaseHistory: Anytime the risk level is HRC; Look through all notes for CaseId and anytime there is an HRC given that CaseID a flag of 1
#then keep the max..
#APcaseh=APcaseh%>%
  #rename(CasehCreatedDate=CreatedDate)

#AP_caseh_df=merge(APcas_acc,APcaseh,by="CaseId",all.x=TRUE) #keep all the original cases even if they do not have form data
#length(unique(AP_caseh_df$CaseId))

#We currently do not know the likelihood of suicide for those assessed at these risk levels using our SSS assessment criteria. As a proxy estimate, we will assume the same likelihood of a suicide attempt in the next 30 days for those who screen at high risk on the C-SSRS. Thus, we will assign a partial PA value of 0.08 for the first month, and will increase the indicator by 0.08 for every subsequent month the client remains at high suicide risk.

#PA==min(max(PA,sumdaysdiffinrisklevelchange*0.08),1) (for each client/account ID) and PADate==lag(CreatedDate)(on the latest/most recent row where NewValue==""HRC"" for the client) and PA09==1"

#AP9_form_df = AP_caseh_df %>% 
##  group_by(CaseId) %>%
# mutate(AP09 = max(case_when((NewValue=="HRC") & 
#   unclass(date(Last_Connection_DateTime__c)-date(CasehCreatedDate))/30>=12 ~ .16, 
#   (NewValue=="HRC") & 
#   unclass(date(Last_Connection_DateTime__c)-date(CasehCreatedDate))/30<12 ~ #(0.08/12*(unclass(date(Last_Connection_DateTime__c)-date(CasehCreatedDate))/30)+.08), 
#   (NewValue=="MRC") & 
#  unclass(date(Last_Connection_DateTime__c)-date(CasehCreatedDate))/30>=12 ~ .04, 
#   (NewValue=="MRC") & 
#   unclass(date(Last_Connection_DateTime__c)-date(CasehCreatedDate))/30<12 ~ #(0.03/12*(unclass(date(Last_Connection_DateTime__c)-date(CasehCreatedDate))/30)+.01)))) %>%
#  filter(AP09>0) %>%
#    group_by(CaseId) %>%
#    filter(AP09==max(AP09)) %>%
#    filter(CasehCreatedDate == min(CasehCreatedDate)) %>%
 #   rename(AP09.Date=Last_Connection_DateTime__c) %>%
 #   select(CaseId, AP09.Date, AP09) 

#unique(AP9_form_df$CaseId)
