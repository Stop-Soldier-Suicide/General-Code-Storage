...
gv_startdate="2023-10-01T05:00:00Z" #This day is included in the range  ## Since it pulls based on the time in England, I allow 5 extra hours to align with midnight EST
...
freq(txcases$Status_Change_Reason__c[txcases$initialrisklevel2=="1. HRC" & txcases$Status=="Closed"])
#ctable(txcases$Status_Change_Reason__c,txcases$initialrisklevel2)

freq(txcases$Status_Change_Reason__c[txcases$initialrisklevel2=="2. MRC" & txcases$Status=="Closed"])
...
ctable(txaccounts$initialrisklevel2,as.factor(txaccounts$crp_cp))
ctable(txaccounts$initialrisklevel2,as.factor(txaccounts$crp))
ctable(txaccounts$initialrisklevel2,as.factor(txaccounts$contingencyplan))
...
ctable(txaccounts$initialrisklevel2,as.factor(txaccounts$cams_bcbt)) ## use initial risk level
ctable(txaccounts$initialrisklevel2,as.factor(txaccounts$cams))
ctable(txaccounts$initialrisklevel2,as.factor(txaccounts$bcbt))
...
feedbackformdata=feedbackformdata%>%
  ...
  rename(Case__c=Care_Plan__c)%>%
  filter(row_number()==1)%>%
  mutate(Promoter=(FSS_NPS_Question__c==9 | FSS_NPS_Question__c==10)*1,
         Detractor=(FSS_NPS_Question__c %in% c(1,2,3,4,5,6))*1)
